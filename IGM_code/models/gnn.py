...
class GNN(torch.nn.Module):
    ...
    def forward(self, batched_data, get_rep=False,alpha=0.2,if_mix=False,num_classes=3):
        h_node = self.gnn_node(batched_data)
        h_graph = self.pool(h_node, batched_data.batch)
        # TODO: add mixup part
        ...
        if if_mix:
            ...
            p = torch.randperm(h_graph.shape[0])
            # print(p.shape)
            # print(batched_data.y.shape)
            # print(h_node.shape)
            # print(h_graph.shape)
            h_mix = lam * h_graph + (1 - lam) * h_graph[p]
            # print(batched_data.y.shape)
            # print(batched_data.y)
            onehot_y = torch.functional.F.one_hot(batched_data.y.long(),num_classes=num_classes).to(device)
            y_mix = lam * onehot_y+ (1 - lam) * onehot_y[p]
            return self.graph_pred_linear(h_mix),y_mix
        if get_rep:
